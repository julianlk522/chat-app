...
export const getUserContacts = asyncHandler(async (req, res) => {
	...
	const sql = `SELECT name, user_id, prefered_pic, messages.content, messages.created_at, messages.seen, messages.sender_id FROM users JOIN messages ON (user_id = messages.sender_id OR user_id = messages.receiver_id) WHERE user_id != ${req.params.userId} AND (messages.sender_id = ${req.params.userId} OR messages.receiver_id = ${req.params.userId}) GROUP BY name;`
