  ...
  db.query(
    ...
    (err, results) => {
      ...
      if (!Number(Object.values(results[0]))) {
        ...
        res.status(400).json({
          message: 'Error: user already exists with username provided',
        });
      }
    }
  );
      ...
      if (Number(Object.values(results[0]))) {
        ...
        db.query(passCheckSql, (err, results) => {
          ...
          if (Number(Object.values(results[0]))) {
            ...
            res
              .status(400)
              .json({ message: 'Error: invalid password provided' });
          }
        });
      } else {
        res.status(400).json({
          message: 'Error: no user found with login credentials provided',
        });
      }
    }
  );
